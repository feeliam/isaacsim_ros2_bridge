...
from robots.Unitree.Go2.runners.env_adapter import Go2EnvAdapter
...
class Go2Runtime(BaseRuntime):
    ...
    职责：
    - 组织 agent_cfg / env / policy / exporter 的构建顺序
    - 对外提供统一 build() 接口
    """
    ...
    def load_agent_cfg(self):
        self.agent_cfg = self.policy_loader.load_agent_cfg()
    ...
    def build_env(self):
        ...
        self.raw_env = self.env_builder.build(self.env_cfg)
        self.env = Go2EnvAdapter.wrap_for_rsl_rl(
            self.raw_env,
            clip_actions=self.agent_cfg.clip_actions,
        )
        return self.env
    ...
    def build_policy(self):
        ...
        self.policy = self.policy_loader.get_inference_policy(
